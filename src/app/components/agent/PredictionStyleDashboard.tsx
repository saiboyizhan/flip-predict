...
const CATEGORY_KEYS: Record<string, string> = {
  'meme-arena': 'predStyle.catMeme',
  'btc-weather': 'predStyle.catBtc',
  'four-meme': 'predStyle.catFourMeme',
  'narrative': 'predStyle.catNarrative',
  'on-chain': 'predStyle.catOnChain',
  'kol': 'predStyle.catKol',
  'rug-alert': 'predStyle.catRug',
  'fun': 'predStyle.catFun',
  'daily': 'predStyle.catDaily',
};

const TAG_COLORS: Record<string, string> = {
  '预测大师': 'bg-amber-500/20 text-amber-400 border-amber-500/30',
...
export function PredictionStyleDashboard({ agentId }: PredictionStyleDashboardProps) {
  ...
  if (loading) {
    return (
      <div className="bg-zinc-950 border border-zinc-800 p-8 flex items-center justify-center">
        <div className="text-zinc-500">{t('predStyle.loadingProfile')}</div>
      </div>
    );
  }
  ...
  if (!profile) {
    return (
      <div className="bg-zinc-950 border border-zinc-800 p-8 text-center text-zinc-600">
        {t('predStyle.noData')}
      </div>
    );
  }
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-zinc-950 border border-zinc-800 p-4">
          ...
          <div className="text-2xl font-bold text-white font-mono">{profile.totalPredictions}</div>
        </div>
        <div className="bg-zinc-950 border border-zinc-800 p-4">
          <div className="flex items-center gap-2 text-zinc-500 text-xs mb-2">
        ...
        <div className="bg-zinc-950 border border-zinc-800 p-4">
          ...
          <div className="text-2xl font-bold text-amber-400 font-mono">{profile.reputationScore}</div>
        </div>
        <div className="bg-zinc-950 border border-zinc-800 p-4">
          <div className="flex items-center gap-2 text-zinc-500 text-xs mb-2">
          ...
          {categoryChartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={categoryChartData}>
                ...
                />
                <Bar dataKey={accuracyKey} fill="#f59e0b" radius={[2, 2, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <div className="h-[200px] flex items-center justify-center text-zinc-600">{t('common.noData')}</div>
          )}
        </div>
      ...
      <div className="bg-zinc-950 border border-zinc-800 p-4 space-y-4">
        <h4 className="text-sm text-zinc-400 flex items-center gap-2">
      ...
      <div className="bg-zinc-950 border border-zinc-800 p-4">
        ...
        <p className="text-zinc-300 text-sm leading-relaxed">{styleSummary}</p>
      </div>
    </motion.div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="relative h-2 bg-zinc-800">
        <div
          className="absolute h-full bg-gradient-to-r from-blue-500 to-amber-500"
          style={{ width: `${pct}%` }}
        />
        <div
          className="absolute top-1/2 -translate-y-1/2 w-3 h-3 bg-white border-2 border-amber-500 rounded-full"
