  ...
  if (!isConnected || !address) {
    return (
      <div className="flex flex-col items-center justify-center py-32 px-6">
        ...
        <h2 className="text-xl font-bold text-muted-foreground mb-2">{t("profile.connectWalletFirst")}</h2>
        <p className="text-muted-foreground text-sm mb-6">{t("profile.connectWalletDesc")}</p>
        <button
          onClick={() => navigate("/")}
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div
        ...
        className="relative overflow-hidden bg-gradient-to-r from-blue-500/10 via-card to-emerald-500/10 border border-border rounded-xl p-6 sm:p-8"
      >
        <div className="flex flex-col sm:flex-row items-center sm:items-center gap-4 sm:gap-6">
          ...
          <div className="min-w-0 text-center sm:text-left">
            <div className="flex items-center justify-center sm:justify-start gap-3 mb-1">
              <h1 className="text-xl sm:text-2xl font-bold text-foreground">{t("profile.player")}</h1>
            ...
            <div className="flex items-center justify-center sm:justify-start gap-2 mb-2">
              ...
              <button
                onClick={handleCopy}
                className="shrink-0 p-1 text-muted-foreground hover:text-blue-400 transition-colors"
                title={address}
              >
                {copied ? (
      ...
      >
        <h2 className="text-xl font-bold text-foreground mb-4">{t("profile.recentActivity")}</h2>
        {recentTrades.length === 0 ? (
          ...
          <div className="bg-card border border-border divide-y divide-border">
            {recentTrades.map((trade) => (
              <div key={trade.id} className="px-4 py-3 flex items-center justify-between hover:bg-accent/50 transition-colors">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-sm text-foreground truncate max-w-[200px] sm:max-w-[300px]">
      ...
      {/* ========== Recent Positions ========== */}
      <div>
        <h2 className="text-xl font-bold text-foreground mb-4">{t("profile.recentPositions")}</h2>

        {positions.length === 0 ? (
          <div className="bg-card border border-border p-16 flex flex-col items-center justify-center">
            <Inbox className="w-16 h-16 text-muted-foreground mb-4" />
            <h3 className="text-xl font-bold text-muted-foreground mb-2">{t("profile.noPositions")}</h3>
            <p className="text-muted-foreground text-sm">{t("profile.noPositionsDesc")}</p>
          </div>
        ) : (
