...
"""
创建穆王八骏精灵图
从192帧中选择48帧，创建8x6布局的精灵图
"""

from PIL import Image
import os

def create_muwang_sprite():
    # 源文件夹
    source_dir = "/Users/xiaobai/pr/9"

    # 输出文件
    output_file = "/Users/xiaobai/Desktop/Premium Horse Racing DApp UI (Copy) 2/public/horse-sprite-muwang.png"

    # 获取所有帧文件（192帧）
    frame_files = sorted([f for f in os.listdir(source_dir) if f.endswith('.png')])
    print(f"找到 {len(frame_files)} 帧")

    if len(frame_files) != 192:
        print(f"警告：预期192帧，实际找到{len(frame_files)}帧")

    # 从192帧中均匀选择48帧
    # 每隔 192/48 = 4 帧选一帧
    selected_indices = [int(i * 192 / 48) for i in range(48)]
    ...
    print(f"\n精灵图参数：")
    print(f"- 文件名: horse-sprite-muwang.png")
